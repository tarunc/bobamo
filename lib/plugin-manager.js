...
var PluginManager = function (options, express) {
    ...
    this.loadRoutes();
    this.configure();
    // console.log('Printing', this.appModel, this.appModel.header)
}
...
PluginManager.prototype.configure = function (req) {
  function doAfter(obj) {
    ...
    var obj = this.persist.read(this.configFile, req, function (err, obj) {
      doAfter(obj);
    });
 ...
 */
PluginManager.prototype.defaultPlugins = [ 'static', 'session', 'appeditor', 'modeleditor', 'imageupload', 'less', 'generator', 'rest', 'mongoose', 'package', 'jsonschema'];
/**
 *
 */
...
PluginManager.prototype.pluginNames = function (json) {
    var p = _u(this.plugins).map(function (plugin) {
        return plugin.name
    });
